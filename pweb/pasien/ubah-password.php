<?php
        ...
        $_SESSION['id'];
        $cekuser = mysqli_query($koneksi, "SELECT * FROM pasien where id_pasien = '$_SESSION[id]'");
        $tampil = mysqli_fetch_object($cekuser);
        // $querycek = mysqli_query($koneksi, $cekuser);
        // $datatampil = mysqli_fetch_object($querycek);
        // $count = mysqli_num_rows($datatampil);
        // if($count >= 1){
...
<div class="container">
        ...
        <div class="konten">
            <p class="tema"><b>UBAH PASSOWRD</b></p>
            <form action="" method="post">
                <div class="kotak1">
                    <label class="usern">Username:  <b><?=$tampil->username;?></b></label>
                    <label for="">Password Lama</label>
                    <input type="password" name="passwordL" class="input">
                    <label for="">Password Baru</label>
                    <input type="password" name="passwordB" class="input">
                    <label for="">Konfirmasi Password</label>
                    <input type="password" name="konfpass" class="input">
                <div class="kotak2">
                    <input type="submit" name="Ganti" onClick="confirm('Anda yakin ingin mengubah password?')" 
                    class="button" value="Kirim">
                    <input type="button" name="" onClick="window.location.href='ubah-nama.php'" class="button" value="Kembali">
                     </div>
                    </div>
            </form>
            <?php
            if(isset($_POST['Ganti'])){
                ...
                if($passwordl != $tampil->password){
                    echo "<script>alert('password Lama anda salah');</script>";
                }else{
                $update = "UPDATE pasien SET  password = '$passwordb' where id_pasien='$_SESSION[id]'";
